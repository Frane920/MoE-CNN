...
class GPUAug:
    ...
    def _batch_rotate(self, img, angles_deg):
        B, C, H, W = img.shape
        angles = -angles_deg * math.pi / 180.0  # tensor or scalar
        if not isinstance(angles, torch.Tensor):
            angles = torch.tensor([angles], device=img.device, dtype=img.dtype).repeat(B)
        angles = angles.to(img.device).to(img.dtype)
    ...
    def add_noise(self, img):
        ...
        if not isinstance(img, torch.Tensor):
            return img
        noise = torch.randn_like(img) * float(self.noise_std)
        out = img + noise
    ...
    def random_erease(self, img):
        ...
        B, C, H, W = img.shape
        device = img.device
        mask = (torch.rand(B, device=device) < self.erase_p)
        if mask.sum() == 0:
            return img
    ...
    def __call__(self, img):
        ...
        if not isinstance(img, torch.Tensor):
            raise ValueError("GPUAug expects a torch.Tensor input [B,C,H,W].")
        ...
        try:
            ...
            if self.brightness > 0 and B > 0:
                fac = torch.empty((B, 1, 1, 1), device=device, dtype=dtype).uniform_(1 - self.brightness, 1 + self.brightness)
                img = img * fac

            if self.contrast > 0 and B > 0:
                fac = torch.empty((B, 1, 1, 1), device=device, dtype=dtype).uniform_(1 - self.contrast, 1 + self.contrast)
                mean = img.mean(dim=[2, 3], keepdim=True)
                img = (img - mean) * fac + mean

            if self.rotate > 0 and B > 0:
                angles = (torch.rand(B, device=device, dtype=dtype) - 0.5) * 2.0 * float(self.rotate)
