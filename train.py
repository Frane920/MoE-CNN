...
class Trainer:
    ...
    def train_one_epoch(self, epoch):
        """Train the model for one epoch with correct DDP no_sync + gradient accumulation."""
        ...
        while True:
            ...
            # Choose no_sync context correctly: must wrap forward+backward
            ...
            with sync_ctx:
                with torch.cuda.amp.autocast(enabled=use_amp):
                    ...
                    total_loss = cls_loss + self.args.penalty_weight * penalty
                ...
                with torch.no_grad():
                    ...
                    penalty_log_val = penalty.detach().cpu().item() if isinstance(penalty, torch.Tensor) else float(
                        penalty)
            ...
            if accum_step:
                if hasattr(self.scaler, "unscale_"):
                    try:
                        ...
                        # some optimizers/state combinations may fail; ignore and continue
                        pass
                ...
                try:
                    ...
                    # fallback to plain step (ensure grads are unscaled)
                    self.optimizer.step()
                    try:
                        self.scaler.update()
                    except Exception:
            ...
            if self.args.rank0:
                ...
                pbar.set_postfix(loss=f"{avg_loss:.6f}", penalty=f"{avg_pen:.15f}", lr=f"{lr:.2e}")
...
def create_training_components(model, args):
...
def main():
    ...
    criterion, optimizer, scheduler, scaler = create_training_components(model, args)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        if args.distributed and dist.is_initialized():
            train_loader.sampler.set_epoch(epoch).set
        ...
        if args.rank0:
            ...
            if is_best:
                if args.export_safetensort:
                    save_safetensors(model, "best"+str(Path(args.save) / "best_model.pt"))
                save_checkpoint(checkpoint, "best"+str(Path(args.save) / "best_model.pt"))

            save_checkpoint(checkpoint, str(Path(args.save) / "latest_model.pt"))
    ...
    if trainer.ema is not None and args.rank0:
        ...
        save_checkpoint(final_state, Path(args.save).with_name("final_model.pt"))
